...
//save size and name of current process to addr
void pcbp(int addr) {
...
//load all needed information of stopped process to a new process
void pcbload(struct proc* t, void* pgtable, struct context* cptr, struct trapframe *tfptr, void* flagptr) {
	struct proc *p;
	p = allocproc();
	
	//set pagetable from stopped process's pages
	p->pgdir = setpagetable(pgtable, flagptr, t->sz);

	//set needed information to new process pcb
...
//save pages, context, trapframe & flags of pages of current process to input pointers
	...
	uint flags;
 	pte_t *pte;
  	...
  	for(i = 0; i < 45056; i += PGSIZE){
		...
		//save pages & flags to input pointers
